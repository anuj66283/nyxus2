...
class FilterView(View):
    ...
    def post(self, request):
        ...
        context['books_reviews'] = zip(books, rv_total, len_rv)
...
class DisplayView(LoginRequiredMixin, View):
    def get(self, request, bid):
        book = get_object_or_404(Books, pk=bid)
        user = request.user

        usr_book = Checkout.objects.filter(user=user, book=book)

        if usr_book:
            return render(request, "display.html", {'book':book})

        else:
            return HttpResponse("Purchase book first")
